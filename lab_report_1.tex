...
\begin{document}
	...
	\begin{item}
		...
		\begin{algorithmic}[1]
			...
			\begin{algsubstates}
				\State For each left node i, if once reinserted, “Find” operation will be used to select  the connected component indices of neighborhoods nodes around node i one by one in the disjoint-set data structure. We mark the unique number of connected component indices of neighborhoods nodes around node i as $N_i$.
				
				\State Find the smallest $N_min$ value of node Min among all left nodes $N_i$, we reinsert the corresponding node Min into the network by the “Union” operation. It will update the connected component indices information in the disjoint-set data structure as well.
				
				\State As mentioned above, when implementing the reinsertion, we add back a finite fraction of nodes at each step. Here top 0.1\% qualified nodes are added back. Let’s say, if we have 2000 nodes left, reinsertion will add back $0.1\%*2000 = 20$ nodes  at each step until all nodes stay in the network again. Hence, at each reinsertion we need to choose 20 nodes in the total 2000 candidates by the value $N_min$ in the $N_i$ array. CI\_HEAP uses directly quick sort algorithm O(Nlog N) to sort all nodes and get the top ones. ComplexCi uses c++ internal STL introselect algorithm (nth\_element) \cite{cppnthelement}\cite{wikiIntroselect} to pick up top N qualified nodes without sort algorithm, which just uses O(N). Because we don’t need to know the order of the $N_min$ array by using full sort and just need to know the top N qualified ones.
				
				\State Repeat to find the next reinsert node in step 2 until all left nodes are consumed.
				\end{algsubstates}
			\end{algorithmic}
		\end{algorithm}
